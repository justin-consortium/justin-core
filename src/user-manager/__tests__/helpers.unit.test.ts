...
/**
 * user-manager helpers unit tests
 *
 * Every function in helpers.ts is a pure transformation — no side effects,
 * no dependencies. Tests here are exhaustive by design: these helpers are
 * called throughout the manager and getting them wrong causes silent bugs
 * upstream.
 */
...
describe('isPlainObject', () => {
  ...
  // Note: isPlainObject checks typeof === 'object', not null, not array.
  // It does not distinguish plain object literals from class instances like Date —
  // that is intentional. The function is used as a lightweight shape guard,
  // not a deep prototype check.
});
...
describe('setValueAtPath', () => {
  ...
  it('returns the source unchanged when path is empty', () => {
    const source = { steps: 100 };
    const result = setValueAtPath(source, '', 999);
...
describe('deleteValueAtPath', () => {
  ...
  it('returns the source unchanged when path is empty', () => {
    const source = { a: 1 };
    const result = deleteValueAtPath(source, '');
